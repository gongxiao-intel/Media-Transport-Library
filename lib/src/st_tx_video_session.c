...
static inline double pacing_tr_offset_time(struct st_tx_video_session_impl* s,
                                           uint64_t epochs) {
  return epochs2time(s, epochs) + s->pacing.tr_offset -
         (s->pacing.tr_offset_vrx * s->pacing.trs);
}
...
static int tv_init_pacing(struct st_main_impl* impl, struct st_tx_video_session_impl* s) {
  ...
  pacing->cur_epochs = time2epochs(s, st_get_ptp_time(impl, ST_PORT_P));
  ...
  info("%s[%02d], trs %" PRIu64 " trOffset %f warm pkts %u\n", __func__, idx, pacing->trs,
...
static int tv_sync_pacing(struct st_main_impl* impl, struct st_tx_video_session_impl* s,
  ...
  if (required_tai) {
    uint64_t ptp_epochs = time2epochs(s, ptp_time);
    epochs = time2epochs(s, required_tai);
  ...
  /* epoch resloved */
  double ptp_tr_offset_time = pacing_tr_offset_time(s, epochs);
  ...
  if (to_epoch_tr_offset < 0) {
    ...
    epochs++;
    ptp_tr_offset_time = pacing_tr_offset_time(s, epochs);
    to_epoch_tr_offset = ptp_tr_offset_time - ptp_time;
  }
  ...
  pacing->cur_time_stamp = pacing_time_stamp(s, epochs);
...
static void tv_stat(struct st_tx_video_sessions_mgr* mgr,
  ...
  info(
      ...
      m_idx, idx, s->ops_name, s->stat_avg_tpr_inter, s->stat_min_tpr_inter, s->stat_max_tpr_inter);
  s->stat_avg_tpr_inter = 0;
  s->stat_min_tpr_inter = 0;
  s->stat_max_tpr_inter = 0;
  s->stat_tpr_inter_cnt = 0;
